// ── Void Rush 3D — game data ────────────────────────────────────
// The map, lanes, towers, enemies and waves. Pure data + tiny helpers,
...
// Three well-separated lanes (caves on the left) feeding a shared castle on
// the right. Corners are axis-aligned; bridges are auto-placed where a lane
// crosses a river column.
const PATH_CORNERS = [
  [[0, 3],  [4, 3],  [4, 6],  [8, 6],  [8, 2],  [14, 2], [14, 6], [19, 6], [19, 8]],  // lane 1 (top)
  [[0, 8],  [5, 8],  [5, 11], [11, 11],[11, 8], [16, 8], [19, 8]],                    // lane 2 (mid)
  [[0, 13], [6, 13], [6, 9],  [12, 9], [12, 13],[17, 13],[17, 8], [19, 8]],           // lane 3 (bottom)
];
...
// ordered grid cells for each lane (used to build enemy waypoints)
...
// river / bridges — a river column cell is a bridge where a lane crosses it,
// otherwise it is water (extended past the map so the river runs off-screen).
...
for (const c of RIVER_COLS) {
  for (let r = -12; r < MAP_ROWS + 12; r++) {
    const key = `${c},${r}`;
    if (pathKey.has(key)) bridgeCells.add(key);   // a lane crosses here -> bridge
    else waterCells.add(key);                     // otherwise -> water
  }
}
export const isWater  = (c, r) => waterCells.has(`${c},${r}`);
export const isBridge = (c, r) => bridgeCells.has(`${c},${r}`);

// ── towers ── costs nudged up (a little) so early gold doesn't buy a full board
...
// ── enemies ── leak = base HP lost if this enemy reaches the castle.
// A bit tougher than the original numbers (tanks/bombers used to melt too
// fast) but dialed back hard from the first rebalance pass, which made the
// tank/bomber waves basically unwinnable.
...
// ── waves ── progressive: ramp count, speed, lanes (1→2→3) and enemy
// variety (drone → tank → ghost → bomber). speed = enemy speed multiplier.
export const WAVES = [
...
// tower level 1/2/3 damage (and, for Frost, slow-strength) multiplier
export const LEVEL_DMG_MULT = [1, 1.5, 2.2];
// tower level 1/2/3 range multiplier
export const RANGE_MULT = [1, 1.12, 1.22];
// spike trap: placed on path tiles, burst damage on contact then a cooldown
